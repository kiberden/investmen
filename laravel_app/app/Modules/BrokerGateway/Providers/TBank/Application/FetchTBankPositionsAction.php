...
final class FetchTBankPositionsAction
{
    ...
    public function execute(Broker $broker, string $accountId): array
    {
        ...
        if ($normalizedAccountId === '') {
            Log::warning('FetchTBankPositionsAction got empty accountId.', [
                'broker_id' => (int) $broker->getKey(),
                'provider_code' => (string) $broker->getAttribute('provider_code'),
                'operation' => 'getPositions',
            ]);

            throw new RuntimeException('T-Bank accountId is required for positions request.');
        }
        ...
        try {
            ...
            return $this->operationsRestClient->getPositions(
            ...
            Log::error('FetchTBankPositionsAction failed.', [
                'broker_id' => (int) $broker->getKey(),
                'provider_code' => (string) $broker->getAttribute('provider_code'),
                'operation' => 'getPositions',
                'error' => $e->getMessage(),
            ...
            throw new RuntimeException('Failed to fetch T-Bank positions.', previous: $e);
        }
    }
}
