...
final class FetchTBankPortfolioActionTest extends TestCase
{
    use RefreshDatabase;

    public function test_it_returns_portfolio_payload_for_broker(): void
    {
        $broker = $this->makeBrokerWithCredential();

        Http::fake([
            'https://example.test/rest/*' => Http::response(['totalAmountPortfolio' => ['units' => '100']], 200),
        ]);

        $payload = app(FetchTBankPortfolioAction::class)->execute($broker, 'acc-1');

        $this->assertSame('100', data_get($payload, 'totalAmountPortfolio.units'));
    }
