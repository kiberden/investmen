...
final class FetchTBankPositionsActionTest extends TestCase
{
    use RefreshDatabase;

    public function test_it_returns_positions_payload_for_broker(): void
    {
        $broker = $this->makeBrokerWithCredential();

        Http::fake([
            'https://example.test/rest/*' => Http::response(['securities' => [['figi' => 'BBG000B9XRY4']]], 200),
        ]);

        $payload = app(FetchTBankPositionsAction::class)->execute($broker, 'acc-1');

        $this->assertSame('BBG000B9XRY4', data_get($payload, 'securities.0.figi'));
    }
