...
final class FetchTBankOperationsByCursorAction
{
    ...
    {
        $accountId = data_get($request, 'accountId');

        if (!\is_string($accountId) || trim($accountId) === '') {
            Log::warning('FetchTBankOperationsByCursorAction got invalid accountId.', [
                'broker_id' => (int) $broker->getKey(),
                'provider_code' => (string) $broker->getAttribute('provider_code'),
                'operation' => 'getOperationsByCursor',
            ]);

            throw new RuntimeException('T-Bank accountId is required for operations-by-cursor request.');
        }
        ...
        try {
            ...
            return $this->operationsRestClient->getOperationsByCursor(
            ...
            Log::error('FetchTBankOperationsByCursorAction failed.', [
                'broker_id' => (int) $broker->getKey(),
                'provider_code' => (string) $broker->getAttribute('provider_code'),
                'operation' => 'getOperationsByCursor',
                'error' => $e->getMessage(),
            ...
            throw new RuntimeException('Failed to fetch T-Bank operations by cursor.', previous: $e);
        }
    }
}
